...
public:
  Card(bool isSymbol)
      : isSymbol(isSymbol), visibility(true){};
  virtual ~Card(){};
  bool isSymbolCard()
  {
...
public:
  SymbolCard(string symbol)
      : Card(true), symbol(symbol){};
  string getValue() const;
  void printCard();
  void setVisibility(bool isPublid)
  {
...
public:
  NumberCard(int num, string color)
      : Card(false), num(num), color(color){};
  string getValue() const;
  void printCard();
...
public:
  Character(const string &name, const bool isPlayer,
            const string type); // constructor
  ...
  virtual int biddingChips(const int currChip,
                           const int limitChip) = 0; // 前一人下注的籌碼
  virtual void setBigOrSmall(bool isBig)
  {
    this->bigOrSmall = isBig;
  };
...
Character::Character(const string &name, const bool isPlayer, const string type)
    : isPlayer(isPlayer)
{
  this->name = name;
  this->type = type;
    ...
    while (idx < cardInHand)
    {
        ...
        else if (this->cardArr[idx]->getValue().compare("*") == 0)
        {
            ...
            if (idx >= 3)
            {
                ...
                // idx = 數字
                double first = 0;
                first = stod(this->cardArr[idx - 2]->getValue()) *
stod(this->cardArr[idx]->getValue()); if (frontIsPlus) value += first; else
                    value -= first;
            }
            else
            {
...
{
  cout << setw(10) << left << this->name << " : " << left;
...
public:
  Player(const string &playername)
      : Character(playername, true, "Player"){};
  ~Player(){};
  void sortCard();
...
public:
  ...
  int biddingChips(const int currChip,
                   const int limitChip); // 前一人下注的籌碼
  void throwCard(Card *c);
  void printWinner();
};

Drunkard::Drunkard(const string n)
    : Character(n, false, "Drunkard")
{
...
Rich::Rich(const string n)
    : Character(n, false, "Rich")
{
...
private:
public:
  Math(const std::string &n)
      : Character(n, false, "Math"){};
  ~Math(){};
  void sortCard();
  int biddingChips(const int currChip,
                   const int limitChip); // 前一人下注的籌碼(要先呼叫過sortCard才能呼叫biddingChips)
  void printWinner();
  void throwCard(Card *c);
};
...
int Math::biddingChips(const int currChip, const int limitChip)
{
  ...
  if (this->totalChips > 0) // 還有chips
  {
    ...
    else
    {
      ...
      this->sortCard();
      // modify end
      double result = this->calCard(); // 先取optimal
                                       // solution(要先呼叫過sortCard才能呼叫biddingChips)
...
void Math::sortCard()
{
  ...
  do
  {
    ...
    } while (next_permutation(numberCards.begin(),
                              numberCards.end())); // 產生所有數字卡牌的排列組合
  } while (next_permutation(symbolCards.begin(),
                            symbolCards.end())); // 產生所有符號卡牌的排列組合
...
{
  vector<string> nameList = {"Fourier", "Jay Chou", "Euler", "Ramam", "Newton", "Swift",
                             "Faker", "Lee", "Chen", "Yttria", "AsiaGodTone"};
  ...
  for (int i = 1; i < playerNum; i++)
  {
    ...
    maxNameLen = (maxNameLen >= nameList.back().length() ? maxNameLen : nameList.back().length());
    nameList.pop_back();
  }
  nameList.clear();
...
void Game::calChips()
{
  ...
  cout << this->playerListPerRnd[playerPos]->name << "目前籌碼數量：" << playerListPerRnd[playerPos]->totalChips
       << endl;
...
int main()
{
  ...
  string playerName;
  cin >> playerName;
  maxNameLen = maxNameLen > playerName.length() ? maxNameLen : playerName.length();
  ...
  for (int i = 1; i <= 3; i++)
  {
    ...
    cout << setw(15) << setfill('-') << ""
         << "發基本符號牌三張" << setw(15) << "" << setfill(' ') << endl;
    // G.printPlayersCard();
    G.biddingPerRound(0);
    G.dealCard(1);
    cout << setw(20) << setfill('-') << ""
         << "BID  1" << setw(20) << "" << setfill(' ') << endl;
    cout << "第一輪發牌結束，請確認手牌，並開始第一輪下注" << endl;
    G.biddingPerRound(1);
    G.dealCard(2);
    cout << setw(20) << setfill('-') << ""
